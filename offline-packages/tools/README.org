...
| ~exiftool/~                                          | ~6 MB   | https://sourceforge.net/projects/exiftool/files/Image-ExifTool-13.59.tar.gz/download                              | transmute (image metadata; Perl script + =lib/= tree, needs =perl= on target)                          |
...
For =exiftool=, the tarball extracts to =Image-ExifTool-<ver>/=; strip the top
component so the layout is =exiftool/exiftool= + =exiftool/lib/= (the script
locates its =lib/= relative to =$0=, so it works when invoked via PATH):
#+begin_src bash
cd tools
curl -L -o Image-ExifTool-13.59.tar.gz https://sourceforge.net/projects/exiftool/files/Image-ExifTool-13.59.tar.gz/download
mkdir -p exiftool
tar -xzf Image-ExifTool-13.59.tar.gz --strip-components=1 -C exiftool
chmod +x exiftool/exiftool
rm Image-ExifTool-13.59.tar.gz
#+end_src
...
[[https://github.com/troglobit/mg][mg]] v4.0 source (~457 KB) at ~mg/mg-4.0.tar.gz~.
Extract and build:
...
[[https://github.com/bjasspa/jasspa/releases/tag/me_20260601][official release]].
...
The file ~jasspa-me/meuser.emf~ contains user keybindings and macros; it is
loaded automatically when Jasspa ME starts from the deployed directory.
